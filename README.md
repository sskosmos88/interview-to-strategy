...
> Параметр `--name` — это имя **специалиста**, которому вы делаете стратегию. В примере ниже — «Иван Петров», но вы подставляете реальное имя клиента.
...
| `--name`, `-n` | Имя специалиста, для которого делается стратегия (используется, если не удалось определить автоматически) |
...
Представьте, что вы продюсер и взяли интервью у специалиста Марии Ивановой. Тогда команда будет такой:
...
Здесь:
- `--name` — имя специалиста, которому делается стратегия.
- `--producer-name` и `--producer-telegram` — ваши контакты, чтобы документ подписался как «Подготовил: …».
